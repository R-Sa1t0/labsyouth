...
static inline int decap(struct xdp_md *ctx, struct cfg *vcfg) {	
	...
	struct ipv6hdr *ipv6h = (struct ipv6hdr *)(ethh + 1);
	if (ipv6h->nexthdr != IPPROTO_ROUTING)
		return XDP_PASS;

	struct ipv6_sr_hdr *srh = (struct ipv6_sr_hdr *)(ipv6h + 1);
	if (srh->type != (u8)4)
		return XDP_ABORTED;

	for (int i=0; i<16; i++)
		if (srh->segments[i] != vcfg->decap_sid[i])
			return XDP_ABORTED;

	bpf_xdp_adjust_head(ctx, sizeof(*ethh) + sizeof(*ipv6h) + sizeof(srh_alloc));
